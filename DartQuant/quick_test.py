#!/usr/bin/env python3
"""
一体化测试脚本：完全仿照 DartQuant 的训练流程
1. 收集激活值用于训练 R1 和 R2
2. 训练 R1 旋转矩阵（使用 Whip Loss）
3. 训练 R2 旋转矩阵（per-head，使用 Whip Loss）
4. 融合 LayerNorm + 应用旋转
5. GPTQ 权重量化
6. 评估 PPL
...
@torch.no_grad()
def evaluate_ppl_correct(model, tokenizer, dataset_name='wikitext2', device='cuda', stride=512):
    """
    正确的 PPL 评估方法 - 使用滑动窗口
    
    标准做法：
    1. 使用完整的测试集文本
    2. 使用滑动窗口（stride）而非截断
    3. 只计算新增 token 的 loss（避免重复计算）
    ...
    Args:
        ...
        dataset_name: 数据集名称
        device: 设备
        stride: 滑动步长（通常设为 seqlen // 4 或 512）
    ...
    if hasattr(test_data, 'column_names'):
        ...
        texts = test_data.get('text', list(test_data))
    ...
    logging.info(f"评估数据集: {dataset_name}, 总 token 数: {seq_len}, 序列长度: {seqlen}, 滑动步长: {stride}")
    ...
    for begin_loc in pbar:
        ...
        trg_len = end_loc - prev_end_loc  # 本次需要计算 loss 的 token 数
        
        input_chunk = input_ids[:, begin_loc:end_loc].to(device)
        target_chunk = input_chunk.clone()
        
        # 只计算新增部分的 loss（前面已经计算过的部分设为 -100）
        target_chunk[:, :-trg_len] = -100
        
        with torch.no_grad():
            outputs = model(input_chunk, labels=target_chunk)
            # outputs.loss 是平均 loss
            # 需要转换为该 chunk 的总 NLL
        ...
        if nlls:
            ...
            total_tokens = prev_end_loc - 0  # 实际上是 prev_end_loc
            # 但这里我们需要用正确的 token 数
            current_ppl = torch.exp(total_nll / sum([min(stride, seq_len - i * stride) for i in range(len(nlls))]))
    ...
    # 计算最终 PPL
    total_nll = torch.stack(nlls).sum()
    total_tokens = prev_end_loc  # 总共评估的 token 数
...
def evaluate_ppl_simple(model, tokenizer, dataset_name='wikitext2', device='cuda'):
    """
    简化版 PPL 评估 - 使用 model.forward 的 labels 参数
    适用于未经包装的标准 HuggingFace 模型
    """
    from datasets import load_from_disk
...
# ============================================================================
# R1 旋转矩阵训练（仿照 r1_base_qr.py）
# ============================================================================

class R1_QR(nn.Module):
    """R1 旋转矩阵（可训练）- 完全仿照 r1_base_qr.py"""
...
def train_R1(r1_train_data, args, device='cuda'):
    """
    训练 R1 旋转矩阵 - 完全仿照 r1_base_qr.py 的 train_R1 函数
    ...
    # r1_train_data: [nsamples, seqlen, hidden_size] -> [nsamples * seqlen, hidden_size]
    ...
    for epoch in range(args.r1_epochs):
        ...
        for batch_idx, (batch_samples,) in enumerate(dataloader):
            ...
            # Whip Loss - 完全仿照 r1_base_qr.py
            loss = torch.sum(torch.exp(-outputs.abs()), dim=-1, keepdim=True).mean()
        ...
        log_message = f'R1 Epoch [{epoch+1}/{args.r1_epochs}], Whip Loss: {mean_loss.item():.4f}'
        if scheduler is not None:
            log_message += f', LR: {scheduler.get_last_lr()[0]:.6f}'
        logging.info(log_message)
    
    logging.info("---> R1 训练完成")
...
# ============================================================================
# R2 旋转矩阵训练（仿照 r2_base_qr.py）
# ============================================================================

class R2_Per_Head(nn.Module):
    """R2 Per-Head 旋转矩阵（可训练）- 完全仿照 r2_base_qr.py"""
...
def train_R2_single_layer(o_proj_data, layer_id, args, device='cuda'):
    """
    训练单层的 R2 旋转矩阵 - 完全仿照 r2_base_qr.py 的 train_r2 函数
    ...
    for epoch in range(args.r2_epochs):
        ...
        for batch_idx, (batch_samples,) in enumerate(dataloader):
            ...
            # Whip Loss - 完全仿照 r2_base_qr.py
            loss = torch.sum(torch.exp(-outputs.abs()), dim=-1, keepdim=True).mean() / args.r2_accumulation_steps
            loss.backward()
        ...
        # 打印日志
        mean_loss = torch.stack(loss_log).mean()
        log_message = f'  R2 Layer {layer_id} Epoch [{epoch+1}/{args.r2_epochs}], Loss: {mean_loss.item():.4f}'
        if scheduler is not None:
            log_message += f', LR: {scheduler.get_last_lr()[0]:.6f}'
        logging.info(log_message)
...
def train_all_R2(o_proj_data_dict, args, device='cuda'):
    ...
    logging.info("---> R2 训练完成")
    return r2_dict


# ============================================================================
# 收集激活值（仿照 get_train_data.py）
# ============================================================================

def collect_training_data(model, dataloader, args, device='cuda'):
    """
    收集用于训练 R1 和 R2 的激活值 - 完全仿照 get_train_data.py 的 get_Rs_training_data
    
    Args:
        model: 已融合 LayerNorm 的模型
        dataloader: 校准数据加载器
        args: 参数
        device: 设备
    
    Returns:
        r1_train_data: R1 训练数据 [nsamples, seqlen, hidden_size]
        o_proj_data_dict: {layer_id: o_proj_data} R2 训练数据
    """
    ...
    for idx, decoder_layer in tqdm(enumerate(layers), total=len(layers), desc="收集各层激活值"):
        ...
        if o_proj_inp['idx'] == args.nsamples:
            ...
            o_proj_data = torch.cat(o_proj_data, dim=0)  # [nsamples, seqlen, hidden_size]
...
def apply_rotation_from_memory(model, r1_matrix, r2_dict, args, device='cuda'):
    """
    从内存中应用旋转矩阵到模型（仿照 rotation_utils.rotate_model）
    """
    logging.info("应用旋转矩阵...")
...
def main():
    ...
    logging.info("=" * 60)
    logging.info("DartQuant 一体化测试脚本")
    logging.info("=" * 60)
    ...
    # 使用正确的 PPL 评估方法
    try:
        # 尝试使用简化版评估（适用于 ActQuantWrapper 包装后的模型）
        ppl = evaluate_ppl_simple(model, tokenizer, args.eval_dataset, device)
    except Exception as e:
        logging.warning(f"简化版 PPL 评估失败: {e}")
    ...
    print(f"\n{args.eval_dataset.upper()} PPL: {ppl:.2f}")
